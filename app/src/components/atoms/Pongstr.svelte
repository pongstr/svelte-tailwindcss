<script lang="ts">
  ...
  $: dashboard =
    $workspaceStore && $workspaceStore.current && $workspaceStore.current.uid
      ? ['', 'd', $workspaceStore.current.uid, ''].join('/')
      : '/'
</script>

<a use:link href={dashboard} class="block mt-8 px-3 text-oh-purple">
  <svg viewBox="0 0 312 247" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
